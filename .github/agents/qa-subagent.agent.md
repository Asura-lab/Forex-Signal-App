...
When the task matches, invoke these skills directly:

- `breakdown-test`: formal test strategy and coverage planning
- `polyglot-test-agent`: generating concrete tests that compile and run
- `agentic-eval`: rubric-based quality checks and iterative evaluation
- `doublecheck`: verification workflow for factual or high-stakes claims
- `ai-prompt-engineering-safety-review`: prompt safety, bias, and jailbreak risk analysis

If Team Leader includes `SKILL TO USE: ...`, apply that skill first and then execute QA validation.
