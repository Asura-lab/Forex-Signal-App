...
When a delegated task matches one of these workflows, invoke the skill instead of improvising:

- `aider`: non-trivial multi-file implementation in an existing codebase
- `refactor`: maintainability improvements without behavior changes
- `quasi-coder`: translating shorthand or pseudo-code into working implementation
- `polyglot-test-agent`: generating or expanding unit/integration test coverage
- `create-technical-spike`: decision-heavy research before coding
- `architecture-blueprint-generator`: architecture documentation and consistency checks
- `git-commit`: commit creation (only when the user explicitly asked for a commit)
- `autoresearch`: iterative optimization where measurable metrics exist

If a skill is requested by the Team Leader prompt (`SKILL TO USE: ...`), you must apply that skill first, then continue implementation.
...
5. DELIVER
   ...
   - Explicitly report: `Skill used: <name>` or `Skill used: none`.
```
